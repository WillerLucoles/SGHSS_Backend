...
describe('API de Pacientes', () => {
  // O bloco `beforeEach` garante que a base de dados é limpa antes de CADA teste.
  // Isto é ótimo para garantir que os testes não interferem uns com os outros.
  beforeEach(async () => {
  ...
  it('deve retornar um erro 409 ao tentar registar um paciente com um email que já existe', async () => {
    // Passo 1: Criar um paciente para que o email já exista na base de dados.
    // A função `criarPaciente` já faz o registo e o login.
    await criarPaciente({
    ...
    // Passo 2: Tentar registar um novo paciente com o mesmo email.
    const pacienteComEmailDuplicado = {
      ...
      estado: 'MG',
      cep: '30140000',
      email: 'paciente.duplicado@email.com', // Email repetido
